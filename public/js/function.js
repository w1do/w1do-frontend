(function ($) {
	...
	/* Company Support Slider JS */
	if ($('.company-supports-slider-gold').length) {
		const company_supports_slider_gold = new Swiper('.company-supports-slider-gold .swiper', {
			slidesPerView : 2,
			speed: 2000,
			spaceBetween: 20,
			loop: true,
	...
	$window.on( "load", function(){
		if( $(".project-item-list").length ) {
			...
			var $menudisesnav = $(".our-project-filter-nav li a");
	...
	/* On Scoll Rotate JS */
	let last = scrollY, rot = 0;
	addEventListener('scroll', () => {
	rot += (scrollY - last) * 0.09; // slower
	last = scrollY;
